...
class motion:
    ...
    def finish_callback(self, finish):
        ...
        if finish.x == 1: #if the object is successfully grabbed, we will shutdown this node
        # we will also set vel to 0 to make sure to avoid unwanted motion

            finish_number = finish.x

            self.msg.linear.x = 0
            self.msg.linear.y = float(0.4)
